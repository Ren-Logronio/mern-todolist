...
router.delete('/:id', artificiallyDelay_1.default, auth_1.verifyJWT, (req, res) => {
    const todoListId = req.params.id;
    todoListModel_1.todoListModel.findByIdAndDelete(todoListId).populate("todos").then((todoList) => {
        todoList.todos.forEach((todo) => {
            todoModel_1.todoModel.findByIdAndDelete(todo._id).catch((err) => res.status(500).json(err));
        });
